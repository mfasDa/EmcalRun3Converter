...
{
  std::string url, bcmpath, timepath;
  long timestamp(-1);
  int timeshift(0), runnumber(0);
  boost::program_options::options_description desc("Allowed options");
  desc.add_options()("help", "produce help message")("run", boost::program_options::value<int>(&runnumber)->default_value(0), "Run number to be converted")("timestamp", boost::program_options::value<long>(&timestamp)->default_value(-1), "Output timestamp in CCDB")("timeshift", boost::program_options::value<int>(&timeshift)->default_value(0), "Time shift to be applied in the converion of the time calib params")("url", boost::program_options::value<std::string>(&url)->default_value("emcccdb-test.cern.ch:8080"), "inputfile to be produced with o2 digits")("badchannelpath", boost::program_options::value<std::string>(&bcmpath)->default_value("BadChannels"))("timepath", boost::program_options::value<std::string>(&timepath)->default_value("Time"));
  ...
  if (optionmap.count("badchannelpath")) {
    bcmpath = optionmap["badchannelpath"].as<std::string>();
  }
  if (optionmap.count("timepath")) {
    timepath = optionmap["timepath"].as<std::string>();
  }
  ...
  std::cout << "Running calib converter ..." << std::endl
	    ...
	    << "  Bad channel map:                EMC/" << bcmpath << std::endl
	    << "  Time calib:                     EMC/" << timepath << std::endl
            << "====================================================================" << std::endl;
  ...
  calibconverter.setPathBadChannels(bcmpath.data());
  calibconverter.setPathTime(timepath.data());
  return EXIT_SUCCESS;
}
